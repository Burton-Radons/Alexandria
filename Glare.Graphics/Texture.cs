...
namespace Glare.Graphics
{
	...
	public abstract class Texture : SamplerResource, IResourceSource
	{
		...
		internal Texture(Format format, Vector4i dimensions) : this()
		{
		...
		public static Texture2D CreateRandom4nb(int width, int height)
		{
			...
			texture.Data(width, height, TextureFormats.Vector4nb, data);
			return texture;
		}
		...
		protected void Data2D(int level, Vector2i dimensions, Format storageFormat, IntPtr pointer, Format uploadFormat = null)
		{
			...
			using (Lock())
			{
				GL.TexImage2D(Target, level, storageFormat.pixelInternalFormat, dimensions.X, dimensions.Y, 0, uploadFormat.pixelFormat, uploadFormat.pixelType, pointer);
				Context.CheckError();
			}
		}
		...
		protected void Storage2D(int levels, Format format, Vector4i dimensions)
		{
			...
			using(Lock())
				GL.TexStorage2D((TextureTarget2d)Target, levels, (SizedInternalFormat)format.pixelInternalFormat, dimensions.X, dimensions.Y);
		}
	...
	internal struct TextureLock : IDisposable
	{
		...
		public void Dispose()
		{
			...
			finally { GL.BindTexture(texture.Target, old); }
		}
	}
