...
namespace Glare.Graphics.Terrains.Planar.Components
{
	...
	{
		public HeightTerrainComponent(PlanarTerrain terrain, Format format) : base(terrain, format)
		{
			foreach (PlanarTerrainBlock block in terrain.Blocks)
				block.HeightTexture = Get(block);
		}

		public HeightTerrainComponent(PlanarTerrain terrain) : this(terrain, TextureFormats.Vector1f) { }
