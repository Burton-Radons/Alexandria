...
namespace Alexandria {
	...
	/// </remarks>
	public abstract class ResourceFormat : PluginResource {
		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		protected readonly ArrayBackedList<Type> CreateTypesMutable = new ArrayBackedList<Type>();

		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		protected readonly ArrayBackedList<string> ExtensionsMutable = new ArrayBackedList<string>();

		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		protected readonly ArrayBackedList<Type> LoadTypesMutable = new ArrayBackedList<Type>();

		[DebuggerBrowsable(DebuggerBrowsableState.Never)]
		protected readonly ArrayBackedList<Type> SaveTypesMutable = new ArrayBackedList<Type>();

		/// <summary>Get whether this <see cref="ResourceFormat"/> can create a new <see cref="Resource"/> of the <see cref="PrimaryType"/>.</summary>
		public bool CanCreate { get; private set; }

		/// <summary>Get whether this <see cref="ResourceFormat"/> can load files using <see cref="LoadMatch"/> and <see cref="Load"/>, as well as enumerating the types of <see cref="Resource"/>s it will load in <see cref="LoadTypes"/>.</summary>
		...
		/// <summary>Get the file extensions, including preceding ".", that this <see cref="ResourceFormat"/> saves files as.</summary>
		...
		/// <summary>Get the types of <see cref="Resource"/>s that can be loaded by this format. This can be empty, if this <see cref="ResourceFormat"/> doesn't implement loading.</summary>
		...
		/// <summary>Get the primary type of <see cref="Resource"/> that this <see cref="ResourceFormat"/> deals with.</summary>
		public Type PrimaryType { get; private set; }

		/// <summary>Get the types of <see cref="Resource"/>s that can be saved by this format. This can be empty, if this <see cref="ResourceFormat"/> doesn't implement saving.</summary>
		...
		public ResourceFormat(PluginResource pluginResource, Type primaryType, bool canLoad = false, IList<Type> loadTypes = null, bool canSave = false, IList<Type> saveTypes = null, bool canCreate = false, IList<Type> createTypes = null)
		...
		/// <summary>Create a new <see cref="Resource"/> of the <see cref="PrimaryType"/>.</summary>
		/// <param name="manager"></param>
		/// <param name="resourceType"></param>
		/// <returns></returns>
		public virtual Resource Create(Manager manager, Type resourceType) {
			if (CanCreate)
				throw new InvalidOperationException("This " + typeof(ResourceFormat).Name + " " + GetType().Name + " says it can create resources, but it doesn't work. Bug!");
			throw new InvalidOperationException("This " + typeof(ResourceFormat).Name + " " + GetType().Name + " cannot create resources.");
		}

		public virtual Resource Load(LoadInfo context) {
			if (CanLoad)
				throw new InvalidOperationException("This " + typeof(ResourceFormat).Name + " " + GetType().Name + " says it can load resources, but it doesn't work. Bug!");
			throw new InvalidOperationException("This " + typeof(ResourceFormat).Name + " " + GetType().Name + " cannot load resources.");
		}

		/// <summary>Attempt to match this <see cref="ResourceFormat"/> against the loader.</summary>
		/// <param name="context"></param>
		/// <returns></returns>
		public virtual LoadMatchStrength LoadMatch(LoadInfo context) {
			return LoadMatchStrength.None;
		}

		public virtual void Save(Resource resource, BinaryWriter writer) {
			if (CanSave)
				throw new InvalidOperationException("This " + typeof(ResourceFormat).Name + " " + GetType().Name + " says it can save resources, but it doesn't work. Bug!");
			throw new InvalidOperationException("This " + typeof(ResourceFormat).Name + " " + GetType().Name + " cannot save resources.");
		}

		/// <summary>Check whether the <see cref="Resource"/> can be saved with this <see cref="ResourceFormat"/>.</summary>
		/// <param name="resource"></param>
		/// <returns></returns>
		public virtual SaveCheckResult SaveCheck(Resource resource) {
			if (CanSave)
				throw new InvalidOperationException("This " + typeof(ResourceFormat).Name + " " + GetType().Name + " says it can save resources, but it doesn't implement SaveCheck. A bug!");
			return new SaveCheckResult(false, "This " + typeof(ResourceFormat).Name + " " + GetType().Name + " cannot save resources.");
		}

		public static Resource LoadResource(LoadInfo context, IEnumerable<ResourceFormat> formats, ResolveLoadConflictCallback resolveConflict = null) {
			LoadMatchStrength matchStrength;
			ResourceFormat loader = LoadMatchResource(out matchStrength, context, formats, resolveConflict);
			return loader.Load(context);
		}

		public static ResourceFormat LoadMatchResource(out LoadMatchStrength matchStrength, LoadInfo context, IEnumerable<ResourceFormat> formats, ResolveLoadConflictCallback resolveConflict = null) {
			if (context == null)
				throw new ArgumentNullException("context");
			if (formats == null)
				throw new ArgumentNullException("formats");

			ResourceFormat bestFormat = null;
			...
			foreach (ResourceFormat format in formats) {
				...
				context.Reset();
				LoadMatchStrength match = format.LoadMatch(context);
			...
			if (!bestConflict) {
				if (bestFormat == null)
					throw new InvalidDataException("No loader could be found for " + context.Name + ".");

				return bestFormat;
			}

			// Otherwise there are multiple formats with equal match strengths; gather those together.
			List<ResourceFormat> conflicts = new List<ResourceFormat>();
			...
			if (resolveConflict != null)
				bestFormat = resolveConflict(context, conflicts, matchStrength);

			// If no resolution is found, throw an exception.
			if (bestFormat == null)
				throw CreateConflictException(context, matchStrength, conflicts);

			// A resolution was found, so return the best format.
			return bestFormat;
		}

		public static InvalidDataException CreateConflictException(LoadInfo context, LoadMatchStrength matchStrength, IList<ResourceFormat> formats) {
			string text = string.Format("{3} {0} loaders matched {1} with strength {2}: {4}",
				typeof(ResourceFormat).Name,
				context.Name,
	...
	/// <returns></returns>
	public delegate ResourceFormat ResolveLoadConflictCallback(LoadInfo context, IList<ResourceFormat> formats, LoadMatchStrength matchStrength);
	...
	public class SaveCheckResult {
		...
		/// <summary>Get a simple <see cref="SaveCheckResult"/> that simply can't save the format.</summary>
		public static readonly SaveCheckResult Cannot = new SaveCheckResult(false);

		public SaveCheckResult(bool result, params string[] problems) {
			Result = result;
			Problems = new ArrayBackedList<string>(problems);
		}
	}

	public class LoadInfo {
		public Resource ContextResource { get; private set; }
		...
		public LoaderFileOpener Opener { get; private set; }

		/// <summary>Get the <see cref="BinaryReader"/> to load from.</summary>
		public BinaryReader Reader { get; private set; }

		/// <summary>Get the starting offset in the <see cref="Stream"/> that this <see cref="LoadInfo"/> object was created with.</summary>
		...
		public LoadInfo(BinaryReader reader, string name, LoaderFileOpener opener, Resource context = null, long? length = null) {
			if (reader == null)
		...
		public Stream OpenRead(string name) { return Opener(name, FileMode.Open, FileAccess.Read, FileShare.Read); }
		public BinaryReader OpenReader(string name) { return new BinaryReader(OpenRead(name)); }
		public BinaryReader OpenReader(string name, ByteOrder byteOrder) { return BigEndianBinaryReader.Create(byteOrder, OpenRead(name)); }
	...
	/// <summary>
	/// Match level returned by <see cref="ResourceFormat.LoadMatch"/>. Each is an integer value, and so there can be values in between if necessary.
	/// 
